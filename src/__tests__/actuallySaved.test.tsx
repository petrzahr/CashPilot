...
describe('MonthlyBudgetScreen — nové rozvržení karet', () => {
  ...
  it('karty "Použitelný zůstatek" a "Celkový majetek" se vykreslují nad blokem "Souhrnně: všechny účty" se správnými hodnotami', () => {
    const txs = [
      makeTx({ type: 'income', sourceAccountId: checking.id, amountInHaler: 500000, date: '2026-09-16' }),
    ];
    ...
    // Zvýrazněné karty jsou nad celým blokem "Souhrnně: všechny účty" (badge i grid).
    expect(usableIdx).toBeLessThan(souhrnneBadgeIdx);
    expect(netWorthIdx).toBeLessThan(souhrnneBadgeIdx);
